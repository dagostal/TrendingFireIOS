...
protocol FiredDelegate: class {
    
    func card(_ card: FiredView, wasDraggedWithFinishPercentage percentage: CGFloat, inDirection direction: SwipeResultDirection)
    func card(_ card: FiredView, wasSwipedIn direction: SwipeResultDirection)
    func card(_ card: FiredView, shouldSwipeIn direction: SwipeResultDirection) -> Bool
    func card(cardWasReset card: FiredView)
    func card(cardWasTapped card: FiredView)
    func card(cardSwipeThresholdRatioMargin card: FiredView) -> CGFloat?
    func card(cardAllowedDirections card: FiredView) -> [SwipeResultDirection]
    func card(cardShouldDrag card: FiredView) -> Bool
    func card(cardSwipeSpeed card: FiredView) -> DragSpeed
    func card(cardPanBegan card: FiredView)
    func card(cardPanFinished card: FiredView)
}
...
public class FireView: UIView, UIGestureRecognizerDelegate {
    ...
    weak var delegate: FiredDelegate? {
        //once a fireview is created, the speed to that class is set with consigureSwipeSpeed!!!!
        ////
        didSet {
            configureSwipeSpeed()
            print("A DELEGATE JUST GOT SET")
            print(delegate!)
        }
    }
    ...
    private func setup() {
        panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(FiredView.panGestureRecognized(_:)))
        addGestureRecognizer(panGestureRecognizer)
        panGestureRecognizer.delegate = self
        tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(FiredView.tapRecognized(_:)))
