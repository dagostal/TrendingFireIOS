...
open class KolodaView: UIView, DraggableCardDelegate {
    ...
    private func setupDeck() {
        if let dataSource = dataSource {
            //dataSource is view controller class..this runs when delegate is set..
    ...
    // MARK: Frames
    //setting frame for card.. complicated due to overlap feature that i disabled..
    ...
    func adAdViewToCard(adView:GADUnifiedNativeAdView,nativeAd:GADUnifiedNativeAd) {
        if(self.visibleCards.count == 2) {
            self.visibleCards[1].addAdView(adView:adView,nativeAd:nativeAd)
        }
    }
    
    func card(cardWasTapped card: DraggableCardView,tappedBottomRight: Bool) {
        //get second card in deck, decrease size...
...
//        self.backgroundCardsScalePercent = 0.5
//        self.subviews[0].clipsToBounds = false
//
//        self.subviews[0].frame = CGRect(x: 0, y: 0, width: 5, height: 5)
//        self.subviews[0].bounds = CGRect(x: 0, y: 0, width: 5, height: 5)
//        self.subviews[0].contentMode = .redraw        
        
//        print(self.subviews[0].frame)
//        self.subviews[0].setNeedsDisplay()
//         self.subviews[0].setNeedsLayout()
//        self.setNeedsLayout()
//        self.setNeedsDisplay()

        
        //self.subviews[0].translatesAutoresizingMaskIntoConstraints = false
...
//        self.autoresizesSubviews = true
        ...
        ///8.23 ..anchorimage is the constraint b/t cad and ui image.. it wont help decrease the card size..the card size is calulated using a frame, it is complex due to the overlap feature that i disabled... see frameforcard func\
        
//        visibleCards[1].topAnchorOFCard = visibleCards[1].topAnchor.constraint(equalTo: self.topAnchor, constant: 20)
        
        
        
        //NONE OF THIS IS WORKING BECAUSE IMAGEANCHORVIEW IS DECALRED FOR THE UI IMAGE, but set in the card class.... it does NOT reference THE CARD! FUCK
        
        //THE IMAGE ANCHOR REFERS TO THE UI IMAGE constrinats,BUT IS ONLT ACCESSIBLE FROM THE CARD CLASSS
...
//        To hide, update the .constant of the width constraint to 0.f. The other views will automatically move left to assume position.
        ...
        delegate?.koloda(self, didSelectCardAt: index, tappedBottomRight: tappedBottomRight)
    }
...
//        self.countOfCards = self.countOfCards - 1
    ...
    //function to update countof cards...may areadye exist? called when data source in delegate (view controller) is updated
